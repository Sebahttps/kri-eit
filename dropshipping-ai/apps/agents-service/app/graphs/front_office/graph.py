...
b) Pregunta SIEMPRE la comuna antes de ofrecer forma de pago. En la Región Metropolitana ofrece pago contra entrega. Fuera de la RM ofrece reserva con abono y saldo al recibir; ahí NO ofrezcas contra entrega.
...
async def verificar_disponibilidad(product_id: str, cantidad: int = 1) -> str:
    ...
    if res.en_stock:
        return (f"STOCK CONFIRMADO con el proveedor en {res.latencia_ms} ms "
                f"(disponibles: {res.cantidad or 'suficientes'}). Ya puedes ofrecer el pago: contra entrega si es RM, reserva con abono si es región.")
